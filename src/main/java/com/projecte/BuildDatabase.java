    ...
    public class BuildDatabase {
        ...
        public static void insertAllPokemon(AppData db) {
            ...
            db.update("DROP TABLE IF EXISTS Pokemon;"); // Limpiar la tabla de Pokémon antes de insertar
        ...
        public static void insertAllItems(AppData db) {
            ...
            db.update("DROP TABLE IF EXISTS Item;"); // Limpiar la tabla de objetos antes de insertar
        ...
        public static void insertAllAttacks(AppData db) {
            ...
            db.update("DROP TABLE IF EXISTS Attack;"); // Limpiar la tabla de ataques antes de insertar
            db.update("DROP TABLE IF EXISTS PokemonAttack;"); // Limpiar la tabla de ataques de Pokémon antes de insertar
            ...
            // se asignan todos los ataques anteriores de manera aleatoria dependiendo de su tipo
            ...
            for (HashMap<String, Object> pokeType : pokeTypes) {
                ...
                // Para cada tipo, selecciona hasta 2 ataques aleatorios de ese tipo
                ArrayList<Integer> selectedAttackIds = new ArrayList<>();
                int maxAttacks = 4;
                var attacksPerType = types.length == 2 ? 2 : 4;

                for (String type : types) {
                ArrayList<Integer> matchingAttacks = new ArrayList<>();
                for (HashMap<String, Object> attackType : attackTypes) {
                    int attackId = (int) attackType.get("id");
                    String attackTypeStr = (String) attackType.get("type");
                    if (attackTypeStr.equals(type)) {
                    matchingAttacks.add(attackId);
                    }
                }
                // Selecciona hasta attacksPerType ataques aleatorios de este tipo
                HashSet<Integer> chosen = new HashSet<>();
                while (chosen.size() < Math.min(attacksPerType, matchingAttacks.size())) {
                    int idx = rand.nextInt(matchingAttacks.size());
                    chosen.add(matchingAttacks.get(idx));
                }
                selectedAttackIds.addAll(chosen);
                }

                // Limita el total a maxAttacks
                while (selectedAttackIds.size() > maxAttacks) {
                selectedAttackIds.remove(rand.nextInt(selectedAttackIds.size()));
                }

                // Inserta los ataques seleccionados
